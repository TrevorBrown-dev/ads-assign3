...
public class TrevorB 
{
    public static void main(String[] args) 
    {
        try 
        {
            ...
            Queue<ExpressionEvaluator> expressions = new LinkedList<ExpressionEvaluator>();

            while(scan.hasNextLine()) { expressions.add(new ExpressionEvaluator(scan.nextLine())); }
            while(!expressions.isEmpty()) { 
                ExpressionEvaluator nextExpression = expressions.poll();
                try
                {
                    nextExpression.evaluate(); 

                }catch(Exception e) {
                    System.out.printf("Error found in expression: %s\n", nextExpression);
                    e.printStackTrace();
                }
            }
...
class ExpressionEvaluator 
{
    ...
    public ExpressionEvaluator() { this(""); }
    public ExpressionEvaluator(String expression) { this.expression = expression; }
    ...
    {
        expression = (expression.indexOf("$") == -1) ? expression : expression.substring(0, expression.indexOf("$") + 1);
    ...
    private Integer calculate(String[] expArr) throws IllegalArgumentException
	{
		...
		for (String s : expArr)
		{
			if (s.matches("-?\\d+")) operandStack.push(Integer.parseInt(s));  // push numbers to the stack
			else  // else: expecting operator
			{
				// directional operations need pop's called out of order
				// it is efficient to pop now instead of calling this for every directional operation
				int temp = operandStack.pop();
				if (operandStack.isEmpty() && s.equals("$")) return temp;  // calculation worked
				switch (s)
				{   
					case "+":
						if (operandStack.isEmpty()) throw new IllegalArgumentException("unexpected operator(s)");
					...
					case "/":
						if (operandStack.isEmpty()) throw new IllegalArgumentException("unexpected operator(s)");
						if (temp == 0) throw new IllegalArgumentException("division by 0");
						operandStack.push(operandStack.pop() / temp);
						break;
					case "^":
						if (operandStack.isEmpty()) throw new IllegalArgumentException("unexpected operator(s)");
					...
					case "_":
						...
						break;
					case "#":
						if (temp < 0) throw new IllegalArgumentException("taking sqrt of negative number");
    ...
    private String[] assignVariables(String[] tokens)
    {
        ...
        for(int i = 0; i < tokens.length; i++)
        {
            ...
            {
                if(variables.containsKey(tokens[i])) { tokens[i] = variables.get(tokens[i]); }
                else
                {
